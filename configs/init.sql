...
ALTER TABLE user_groups
    ADD INDEX index_user_groups_name (name) USING BITMAP,
    ADD INDEX index_user_groups_default_policy_id (default_policy_id) USING BITMAP;
...
ALTER TABLE users
    ADD INDEX index_users_group_id (group_id) USING BITMAP;
...
ALTER TABLE buckets
    ADD INDEX index_buckets_name (name) USING BITMAP,
...
ALTER TABLE object_md
    ADD INDEX index_object_md_status (status) USING BITMAP;
...
(
    ...
    created_at      DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'create time',
    status          VARCHAR(32)   NOT NULL DEFAULT 'pending' COMMENT 'write status: pending/completed/failure'
) PRIMARY KEY (bucket_id, object_name, version)
DISTRIBUTED BY HASH(bucket_id) BUCKETS 32 PROPERTIES (
    "bloom_filter_columns" = "id",
    "enable_persistent_index" = "true",
    "replication_num" = "1"
...
ALTER TABLE object_v_md
    ADD INDEX index_object_v_md_status (status) USING BITMAP;
...
ALTER TABLE fragment_md
    ADD INDEX index_fragment_md_status (status) USING BITMAP;
...
ALTER TABLE multipart_uploads
    ADD INDEX index_multipart_uploads_status (status) USING BITMAP;
